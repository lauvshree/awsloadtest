...
<?php
  ...
  try {
    $mysqli = new mysqli($RDS_URL, $RDS_user, $RDS_pwd, $RDS_DB);
 ...
 if ( $mode=="added")
 {
  $mysqli->query("INSERT INTO address (lastname, firstname, phone, email) VALUES ('$lastname', '$firstname', '$phone', '$email')");
 } elseif ( $mode=="edit")
 {
 ...
 {
   $mysqli->query("UPDATE address SET lastname = '$lastname', firstname = '$firstname', phone = '$phone', email = '$email' WHERE id = $id");
   Print "Data Updated!<p>";
 } elseif ( $mode=="remove")
 {
  $mysqli->query("DELETE FROM address where id=$id");
 Print "Entry has been removed <p>";
 }

 $data = $mysqli->query("SELECT * FROM address ORDER BY lastname ASC")
 or die(mysql_error());
